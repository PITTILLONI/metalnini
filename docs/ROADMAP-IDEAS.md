...
2. **Un catalogue de lancement construit autour de quelques têtes d'affiche, pas seulement "qui a dit oui en premier"** — le jeu perd son sens sans des pointures reconnues du metal/rock (voir positionnement : c'est l'authenticité d'un vrai poids culturel qui porte l'engagement, pas juste un vrai nom quelconque). Pas un catalogue complet dès le départ, mais pas un catalogue volontairement mineur non plus : un petit nombre d'artistes "ancres" choisis pour leur poids dans la culture, le reste du catalogue grandissant autour.
3. **Deux vitesses pour atteindre ces ancres, pas un choix entre "stars" et "petits groupes"** :
   - Pour les artistes reconnus, passer par les canaux professionnels (label, management, agent) plutôt que directement par le groupe — c'est le canal normal pour ce type de demande côté business, et souvent plus rapide qu'un message à un petit groupe sans structure claire pour savoir qui doit même répondre.
   - S'appuyer sur la preuve déjà construite (page de pitch, retours de proches, prototype qui tourne) comme argument de crédibilité auprès de ces interlocuteurs — une proposition avec une démo qui fonctionne convainc plus qu'une idée sur papier.
   - Accepter que ça prenne plus de temps et un taux de succès par contact plus bas que viser des groupes émergents — c'est le compromis assumé pour ne pas sacrifier l'impact du jeu.
   - Les artistes indépendants/émergents restent utiles **en complément**, une fois les ancres posées (diversité du catalogue, rareté, actu plus fréquente donc plus de contenu vivant) — en renfort, jamais en remplacement des têtes d'affiche.
4. **Présenter l'app comme un bénéfice pour l'artiste, pas une demande de faveur** — passer de « puis-je utiliser votre image » à « voici un outil d'engagement fan gratuit, qu'on aimerait construire avec vous » change la nature de la conversation, y compris face à un contact professionnel. Amorce possible, à adapter selon l'interlocuteur (le groupe directement ou son management) : *« On construit une app de cartes à collectionner pour les fans de metal/rock, avec [artiste] déjà représenté dans un prototype privé — on adorerait vous montrer ce que ça donne et voir si ça vous intéresse d'en faire partie officiellement, sans coût de votre côté. »* Une prise de contact, pas un contrat — les termes se négocient après un accord de principe, pas avant.
