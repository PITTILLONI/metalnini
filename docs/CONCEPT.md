...
- **Droits à l'image / musicaux** : usage actuel de **vrais noms de groupes/musiciens et de portraits IA dont l'apparence se rapproche des vrais visages** (à partir de photos). Deux risques distincts et cumulatifs : (1) le **droit à l'image** porte sur la personne reconnaissable, indépendamment du fait que l'image soit une photo ou une illustration originale — un style original ne suffit pas à s'en affranchir ; (2) si l'IA s'appuie sur une photo précise, la question de l'**œuvre dérivée** (droits du photographe) se pose selon ce qui subsiste de ses choix créatifs (cadrage, pose, lumière) — terrain juridiquement disputé (cf. le litige de l'affiche "Hope" de Shepard Fairey, basée sur une photo AP précise). **Ce choix n'est acceptable que tant que l'usage reste strictement privé** (pas de diffusion publique/commerciale, pas de mise en avant à grande échelle) — y compris sur la page de pitch, qui est techniquement publique (hébergée sur un repo GitHub public, mot de passe = filtre de politesse et non une vraie confidentialité) : décision assumée et actée pour l'instant par le porteur de projet en connaissance de ce risque, à réévaluer avant toute diffusion plus large. À faire trancher par un juriste avant toute ouverture publique, même limitée.
  **Stratégie retenue pour la suite** (détaillée dans `ROADMAP-IDEAS.md`, section "Ouverture publique") : ne pas viser un accord avec "tous les groupes" avant d'ouvrir — réduire d'abord la ressemblance des portraits par défaut (le levier qui ne dépend d'aucun tiers), puis viser un catalogue de lancement construit autour de quelques **têtes d'affiche reconnues** (pas un catalogue complet, mais pas non plus un catalogue volontairement mineur — le jeu perd son sens sans pointures du metal/rock), atteintes via les canaux professionnels (label, management) plutôt qu'en démarchage direct, en s'appuyant sur la preuve déjà construite (page de pitch, retours de proches) comme argument de crédibilité. Les artistes émergents/indépendants restent utiles en complément, jamais en remplacement des têtes d'affiche. **Les vrais noms ne bougent pas dans ce plan** — un faux groupe n'a pas le même impact, c'est la raison même de ce choix (voir positionnement). Le retour aux noms/portraits détournés reste documenté comme dernier recours si tout le reste échoue durablement, pas comme une étape prévue.
...
## Statut
Concept validé comme base de travail pour la suite (direction visuelle, puis maquettage), enrichi d'une décision structurante (échange en présentiel uniquement), d'un mécanisme de détection/preuve de concert simplifié (billet plutôt que suivi d'artiste + agenda), d'une couche Missions, et d'une stratégie droits à l'image qui remplace "obtenir l'accord de tous les groupes" par un catalogue de lancement restreint et une réduction de la ressemblance des portraits par défaut (voir `ROADMAP-IDEAS.md`). Les idées écartées du MVP ou à explorer plus tard sont conservées dans `ROADMAP-IDEAS.md` pour ne rien perdre.
