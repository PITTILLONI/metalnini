...
L'humour de la DA ("drôle", voir positionnement) doit se voir sur le portrait lui-même, pas seulement dans le nom détourné du groupe : un test sur "Sleeplot" (clin d'œil à Slipknot, voir `assets/carte-exemple-tarot.jpg` et la page de pitch) a validé qu'un gag visuel simple — ici un masque de sommeil brodé "Zzz" à la place du masque effrayant, sur une silhouette par ailleurs reconnaissable (combinaison, patch cousu, pose bras croisés) — porte bien le second degré dans ce cadre vintage, à l'image des jeux de mots sur les arcanes du moodboard de référence (ex. "The Iron Maiden" en clin d'œil au nom du groupe). Le degré de ressemblance exact avec le vrai groupe reste un point à faire trancher par un juriste (voir risques dans `CONCEPT.md`).
