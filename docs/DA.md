...
4. **Le seul asset réellement sur-mesure par carte = le portrait de l'artiste.** Tout le reste (cadre, typo, effets de rareté, accent de genre) est systématique. En V1, ce portrait est **généré par IA dans un style homogène défini par la DA** (pas une vraie photo de musicien réel) — cohérent avec les noms de groupes détournés retenus pour éviter les problèmes de droits (voir `CONCEPT.md`). Ce style généré doit rester assez distinctif pour ne pas passer pour un simple filtre photo, et suffisamment homogène d'une carte à l'autre pour ne pas trahir sa nature générée au premier coup d'œil.
