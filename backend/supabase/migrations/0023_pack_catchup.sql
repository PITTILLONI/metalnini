-- Metalnini — paquets en réserve : les points du jour non utilisés s'accumulent pendant une absence, jusqu'à pack_catchup_days jours (0 ou 1 = pas de report).
-- Réserve tenue sur le profil (daily_balance), créditée à la première demande de chaque jour (heure de Paris).

alter table public.profiles add column if not exists daily_balance int not null default 0 check (daily_balance >= 0);
alter table public.profiles add column if not exists daily_day date;
insert into public.settings (key, value) values ('pack_catchup_days', '7') on conflict (key) do nothing;

-- crédite la réserve des jours écoulés et la renvoie ; usage interne
create or replace function public.accrue_daily(p_user uuid) returns int
...
begin
  ...
  if d is null then   -- premier passage : ce qu'il reste aujourd'hui, comme avant la réserve
    b := greatest(0, v_ppd - coalesce((select sum(case when o.format = 'small' then 1 else 2 end) from public.pack_openings o
...
revoke execute on function public.accrue_daily(uuid) from public, anon, authenticated;

-- points du jour disponibles (réserve comprise) pour le joueur connecté
create or replace function public.daily_points_left() returns int
language plpgsql volatile security definer set search_path = public as $$
begin
  if auth.uid() is null then return 0; end if;
  return public.accrue_daily(auth.uid());
end $$;
...
-- rappel du paquet du jour : seulement aux joueurs qui ont des points en réserve
...
begin
  ...
  loop
    if public.accrue_daily(u) > 0 and public.send_push(u, 'Tes cartes du jour t''attendent', 'Le merch a rouvert : 1 standard ou 2 minis, à toi de choisir.', 'paquet') then n := n + 1; end if;
  end loop;
  return n;
end $$;
