...
- Back-end Supabase `mdnevzmczljycmgbsrsu` : migrations 0001 à 0024 appliquées (depuis 0006, par l'API de gestion avec le seul `supabase-token` : `POST https://api.supabase.com/v1/projects/mdnevzmczljycmgbsrsu/database/query`, l'historique de la CLI s'arrête donc à 0005) ; 45 musiciens, 225 cartes.
...
- En attente : TestFlight (inscription Apple Developer), inscription par e-mail dans l'app iOS, domaine d'e-mail dédié, paquets dédiés aux nouveaux styles, sons (en préparation par un ami : fiche `sons.html`, 25 sons nommés).
